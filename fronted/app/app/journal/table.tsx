...
const JournalTable = () => {
  ...
  const handleJournalEdit = (id: number, content: string) => {
  ...
  const handleDeleteJournal = async (id: number) => {
    ...
    try {
      ...
      const deletedJournalId: object = {
        journal_id: id
      }

      const response = await deleteJournal(deletedJournalId)
      ...
      console.error("Error during JournalInput: ", error);
    }
    finally {
      setDeleting(false)
    }
